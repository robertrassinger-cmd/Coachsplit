...
class _CoachSplitHomeState extends State<CoachSplitHome> {
  ...
  String _csvLine(List<Object?> values) => values.map(_csvValue).join(';');

  String _csvExport({bool excelBom = false}) {
    final event = _event;
    if (event == null) return '';

    final buffer = StringBuffer();
    if (excelBom) buffer.write('\uFEFF');

    final now = DateTime.now();
    buffer.writeln(_csvLine(['CoachSplit Ergebnisexport']));
    buffer.writeln(_csvLine(['Bewerb', event.name]));
    buffer.writeln(_csvLine(['Exportdatum', '${now.day.toString().padLeft(2, '0')}.${now.month.toString().padLeft(2, '0')}.${now.year} ${_clock(now)}']));
    buffer.writeln(_csvLine(['Startintervall', '${event.intervalSeconds}s']));
    buffer.writeln(_csvLine(['Wertung', event.compareByCategory ? 'Altersklassen' : 'Gesamt']));
    buffer.writeln(_csvLine(['Athleten', event.athletes.length]));
    buffer.writeln(_csvLine(['Messpunkte', event.points.length]));
    buffer.writeln();
    ...
    buffer.writeln(_csvLine(['ENDWERTUNG']));
    buffer.writeln(_csvLine(['Gruppe', 'Platz', 'Startnr.', 'Name', 'AK', 'Gesamtzeit', 'Rückstand', 'Abschnitt', 'Abschnittsplatz']));
    for (final row in finalRows) {
      buffer.writeln(_csvLine([
        event.compareByCategory ? row.athlete.category : 'Gesamt',
        row.place,
        row.athlete.bib,
        row.athlete.name,
        row.athlete.category,
        _fmtDuration(row.elapsed),
        row.deltaToLeader == Duration.zero ? '—' : '+${_fmtDuration(row.deltaToLeader)}',
        row.sectionElapsed == null ? '' : _fmtDuration(row.sectionElapsed!),
        row.sectionPlace ?? '',
      ]));
    }
    buffer.writeln();
    ...
    for (final point in event.points) {
      buffer.writeln(_csvLine(['MESSPUNKT', point.name, point.type == PointType.finish ? 'Ziel' : 'Zwischenzeit']));
      buffer.writeln(_csvLine(['Gruppe', 'Platz', 'Startnr.', 'Name', 'AK', 'Gesamtzeit', 'Rückstand', 'Abschnittszeit', 'Abschnittsplatz', 'Abschnittsrückstand']));
      for (final row in _ranking(point)) {
        buffer.writeln(_csvLine([
          ...
          row.sectionElapsed == null ? '' : _fmtDuration(row.sectionElapsed!),
          row.sectionPlace ?? '',
          row.sectionDelta == null || row.sectionDelta == Duration.zero ? '—' : '+${_fmtDuration(row.sectionDelta!)}',
        ]));
      }
      buffer.writeln();
    }

    buffer.writeln(_csvLine(['Erstellt mit CoachSplit']));
  ...
  String _safeExportBaseName() {
    ...
    return 'coachsplit_${safeName}_${DateTime.now().millisecondsSinceEpoch}';
  }

  String _safeExportFileName([String extension = 'png']) => '${_safeExportBaseName()}.$extension';
  ...
  void _downloadText(String content, String fileName, String mimeType) {
    final bytes = Uint8List.fromList(utf8.encode(content));
    _downloadBytes(bytes, fileName, mimeType);
  }

  Future<void> _downloadPngImage() async {
    try {
      final pngBytes = await _createResultsPngBytes();
      _downloadBytes(pngBytes, _safeExportFileName('png'), 'image/png');
      _show('PNG wurde heruntergeladen');
    } catch (e) {
      _show(e.toString().replaceFirst('Bad state: ', ''));
    }
  }

  Future<void> _sharePngImage() async {
    try {
      final pngBytes = await _createResultsPngBytes();
      final fileName = _safeExportFileName('png');
  ...
  Future<void> _exportPngImage() async {
    ...
    if (width >= 900) {
      await _downloadPngImage();
    } else {
      await _sharePngImage();
    }
  }

  void _downloadCsvExport() {
    final content = _csvExport(excelBom: true);
    if (content.trim().isEmpty) {
      _show('Keine CSV-Daten vorhanden');
      return;
    }
    _downloadText(content, _safeExportFileName('csv'), 'text/csv;charset=utf-8');
    _show('CSV wurde heruntergeladen');
  }

  void _downloadHtmlExport() {
    final content = _htmlExport();
    if (content.trim().isEmpty) {
      _show('Keine HTML-Daten vorhanden');
      return;
    }
    _downloadText(content, _safeExportFileName('html'), 'text/html;charset=utf-8');
    _show('HTML wurde heruntergeladen');
  }
          ...
          child: Row(children: [
            NavigationRail(
              ...
              leading: Padding(
                ...
                child: Text('CoachSplit', style: Theme.of(context).textTheme.titleLarge),
              ),
              destinations: const [
                NavigationRailDestination(icon: Icon(Icons.settings), label: Text('Setup')),
                NavigationRailDestination(icon: Icon(Icons.play_arrow), label: Text('Start')),
                NavigationRailDestination(icon: Icon(Icons.touch_app), label: Text('Erfassen')),
                NavigationRailDestination(icon: Icon(Icons.leaderboard), label: Text('Ergebnis')),
              ],
            ),
